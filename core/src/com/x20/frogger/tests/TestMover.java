...
public class TestMover {
    ...
    public void test() {
        ...
        while (player.getPosition().x < 1) {
            ...
            float deltaMoveSpeed = moveSpeed * deltas[i];
            deltaMoveSpeed = mover.accumulate(deltaMoveSpeed);
            Vector2 deltaPos = moveDir.cpy().scl(deltaMoveSpeed);
            //nextPos.add(deltaPos);
            nextPos.x += deltaPos.x;
            player.setPositionRaw(nextPos);
            i = (i + 1) % 4;
        }
        System.out.println(DebugLog.getMaxPrecisionFormat().format(player.getPosition().x));
        assertEquals(player.getPosition().x, 1);
    }
}
